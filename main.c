...
void Add_New_Contact(phone *ptr)
{
    ...
    FILE *file = fopen("load_data_file.txt", "a+");
    fprintf(file, "%s %d\n", ptr->name, ptr->number);
    fclose(file);
...
void Update_Contact(phone *ptr)
{
    ...
    if(flag==1)
    {
        FILE *demo_file=fopen("load_data_file_demo.txt","a+");
        while (temp_f->next != NULL)
        {
            temp_f = temp_f->next;
            fprintf(demo_file, "%s %d\n", temp_f->name, temp_f->number);

        }
        fclose(demo_file);
        remove("load_data_file.txt");
        rename("load_data_file_demo.txt","load_data_file.txt");
    }
    printf("\n\n\t\t\tEnter Any Key To Continue...");
    getch();
    system("cls");
    ...
    else
    {
        ...
        while (ptr->next != NULL) // go to the all node one by one(traversing)
        {
            if (strcmp(ptr->next->name, test_name) == 0) // jei node delete korbo tar ager node theke take refrence korbo
            {
                ...
                break;                  // 4th delete hoye gele loop theke ber hoye jabo
            }
...
void load_data_from_file(phone *ptr)
{
    int number;
    char name[30];
    FILE *file = fopen("load_data_file.txt", "r");
